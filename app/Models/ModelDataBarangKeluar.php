...
class ModelDataBarangKeluar extends Model
{
  ...
  protected $column_order = array(null, 'faktur', 'tglfakur', 'pelnama', 'totalbayar', null);
  protected $column_search = array('faktur', 'tglfakur', 'pelnama');
  ...
  {
    if ($tglawal == '' && $tglakhir == '') {
      $tbl_storage = $this->dt = $this->db->table($this->table)->join('pelanggan', 'idpel=pelid', 'left');
    } else {
      $tbl_storage = $this->dt = $this->db->table($this->table)->join('pelanggan', 'idpel=pelid', 'left')
        ->where('tglfaktur >=', $tglawal)
        ->where('tglfaktur <=', $tglakhir);
